...
def create_income_screen(user, database, screen):
    ...
    Label(text="Your Incomes", font=("Montserrat", 16), bg ="white").grid(row=2, columnspan=4) 
    # display total income
    # create input field for new income
    Label(text="Add new income", font=("Montserrat", 14), bg ="white").grid(row=5, columnspan=4) 
    ...
    add_income_button = Button(screen, text="Add", height="2", width="15", font=("Montserrat", 10), bg ="white", command = lambda : add_new_income(user, database, screen))
    add_income_button.grid(row=7, column=4)
    # table to display current income values
    Label(text="Existing incomes", font=("Montserrat", 14), bg ="white").grid(row=8, columnspan=4) 
...
def show_income_table(screen):
    ...
    for i in incomes:
        n = int(n + 1)
        Label(text = i["Title"] , font=("Montserrat", 10), bg ="white").grid(row=int(10+n), column=1) 
        Label(text = i["Category"] , font=("Montserrat", 10), bg ="white").grid(row=int(10+n), column=2) 
        Label(text = i["Amount"] , font=("Montserrat", 10), bg ="white").grid(row=int(10+n), column=3) 
        total_income = float(total_income) + float (i["Amount"])
    # display total income
    Label(text="Total Income", font=("Montserrat", 14), bg ="white").grid(row=3, columnspan=4) 
    Label(text=total_income, font=("Montserrat", 14), bg ="white").grid(row=4, columnspan=4) 
