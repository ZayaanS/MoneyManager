...
def create_savings_screen(user, database, screen):
    ...
    Label(text="Add new savings item", font=("Montserrat", 14)).grid(row=3, columnspan=4) 
    ...
    add_savings_button = Button(screen, text="Add", height="2", width="15", font=("Montserrat", 10), command = lambda : add_new_budget(user, database, screen))
    add_savings_button.grid(row=5, column=4)
    # table to display current savings values
    Label(text="Existing savings", font=("Montserrat", 14)).grid(row=6, columnspan=4) 
